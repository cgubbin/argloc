use apm::{ArgumentConfig, HolomorphicFunction, RootEstimateKind, find_roots};
...
fn assert_root_near(roots: &[apm::RootEstimate<Complex<f64>>], expected: Complex<f64>) {
    let best = roots
        .iter()
        .min_by(|a, b| {
...
fn finds_two_real_roots_of_quadratic() {
    ...
    let result = find_roots(Quadratic, domain, config()).unwrap();
...
fn finds_roots_of_unity() {
    ...
    let result = find_roots(CubicUnity, domain, config()).unwrap();
...
fn handles_clustered_roots_and_an_isolated_root() {
    ...
    let result = find_roots(ShiftedCluster, domain, config()).unwrap();
...
fn reports_multiplicity_for_repeated_root() {
    ...
    let result = find_roots(DoubleRoot, domain, config()).unwrap();
...
fn recovers_when_root_lies_on_uniform_split_line() {
    ...
    let result = find_roots(BoundaryRoot, domain, config()).unwrap();
...
fn returns_no_roots_when_region_contains_none() {
    ...
    let result = find_roots(NoRoots, domain, config()).unwrap();
